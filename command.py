...
@click.option('--resume', is_flag=True, help="Resume interrupted download (skip already-downloaded fragments)")
@click.option('--verbose', '-v', is_flag=True, help="Enable verbosity")
def main_downloader(input, username, password, proxy, res, resR, mux, muxfile, keep_, output, resume, verbose):
    ...
    # Retry adapter: handles transient SSL/connection errors at the transport layer
