...
class AbemaTVDownloader:
    def __init__(self, url, session, resume=False):
        self.key = None
        self.iv = None
        ...
        # Use deterministic temp dir for resume support
        if resume:
            url_hash = hashlib.md5(url.encode()).hexdigest()[:12]
            self.temporary_folder = os.path.join(self.yuu_folder, f'resume_{url_hash}') + sffx
    ...
    def download_chunk(self, files, key, iv):
        ...
        iv_bytes = unhexlify(self.iv)
        self.downloaded_files = [None] * len(files)

        # Check for already-downloaded fragments (resume support)
        existing_indices = set()
        for i, tsf in enumerate(files):
            outputtemp = self.temporary_folder + os.path.basename(tsf)
            if '?tver' in outputtemp:
                outputtemp = outputtemp[:outputtemp.find('?tver')]
            if os.path.isfile(outputtemp) and os.path.getsize(outputtemp) > 0:
                existing_indices.add(i)
                self.downloaded_files[i] = outputtemp

        remaining = [i for i in range(len(files)) if i not in existing_indices]
        if existing_indices:
            yuu_log.info(f'Resume: {len(existing_indices)}/{len(files)} fragments already downloaded, skipping')

        if not remaining:
            return self.downloaded_files

        def _download_single(index):
            tsf = files[index]
            outputtemp = self.temporary_folder + os.path.basename(tsf)
            if '?tver' in outputtemp:
                outputtemp = outputtemp[:outputtemp.find('?tver')]

            aes = AES.new(self.key, AES.MODE_CBC, IV=iv_bytes)

            for attempt in range(3):
                try:
                    if attempt > 0:
                        time.sleep(attempt * 2)
                    vid = self.session.get(tsf, timeout=15)
                    decrypted_vid = aes.decrypt(vid.content)
                    os.makedirs(os.path.dirname(outputtemp), exist_ok=True)
                    with open(outputtemp, 'wb') as outf:
                        outf.write(decrypted_vid)
                    return index, outputtemp
                except Exception as err:
                    if attempt == 2:
                        yuu_log.error(f'Failed to download fragment {index} after 3 attempts\nreason: {err}')
                        return index, None

        try:
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {executor.submit(_download_single, i): i for i in remaining}

                with tqdm(total=len(remaining), desc='Downloading', ascii=True, unit='file') as pbar:
                    pbar.update(len(existing_indices))
                    for future in as_completed(futures):
                        idx, result_path = future.result()
                        if result_path is None:
                            self.downloaded_files[idx] = None
                        else:
                            self.downloaded_files[idx] = result_path
                        pbar.update()
        ...
        # Retry failed fragments one more round
        failed = [i for i in remaining if self.downloaded_files[i] is None]
        if failed:
            yuu_log.warning(f'Retrying {len(failed)} failed fragments after 5s delay...')
            time.sleep(5)
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = {executor.submit(_download_single, i): i for i in failed}
                for future in as_completed(futures):
                    idx, result_path = future.result()
        ...
        # Final check
        still_failed = [i for i in range(len(files)) if self.downloaded_files[i] is None]
        if still_failed:
            yuu_log.error(f'{len(still_failed)} fragments still failed after retry: {still_failed}')
            return None
...
class AbemaTV:
    ...
    def get_downloader(self):
        ...
        """
        return AbemaTVDownloader(self.url, self.session, getattr(self, 'resume', False))
    ...
    def parse(self, resolution=None, check_only=False):
        ...
        if is_channel(self.url):
            ...
            if re.match(r'第\d+話\s*(.+)', epnumber_tmp):
                ...
                match = re.match(r'#\d+\s*(.+)', epnumber_tmp)
                if match:
                    eptle = match.group(1)
                else:
                    eptle = epnumber_tmp # 如果匹配不到，直接使用原始标题
            else:
                before_space = epnumber_tmp.split(" ")[0]
                after_space = " ".join(epnumber_tmp.split(" ")[1:])
                if any(char.isdigit() for char in before_space):
